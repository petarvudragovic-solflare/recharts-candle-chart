import {
  tc
} from "./chunk-FCOLVMQE.js";
import "./chunk-QFCKH4KW.js";
import "./chunk-JWKNVOR2.js";
import {
  Ir,
  Ke,
  aa,
...
// node_modules/@storybook/react/dist/chunk-JQQVJC7C.mjs
...
__export(entry_preview_argtypes_exports, { argTypesEnhancers: () => argTypesEnhancers, parameters: () => parameters });
...
function generateArray({ inferredType, ast }) {
  ...
  if (depth <= 2) {
    ...
    if (!wt(compactArray)) return w(compactArray);
  }
  return w(ARRAY_CAPTION, generateArrayCode(ast));
}
function generateObject({ inferredType, ast }) {
  let { depth } = inferredType;
  if (depth === 1) {
    let compactObject = generateObjectCode(ast, true);
    if (!wt(compactObject)) return w(compactObject);
  }
  return w(OBJECT_CAPTION, generateObjectCode(ast));
}
...
function generateFunc({ inferredType, ast }) {
  ...
  if (identifier != null) return w(getPrettyIdentifier(inferredType), generateCode(ast));
  let prettyCaption = generateCode(ast, true);
  return wt(prettyCaption) ? w(FUNCTION_CAPTION, generateCode(ast)) : w(prettyCaption);
}
function generateElement(defaultValue, inspectionResult) {
  ...
  if (identifier != null && !isHtmlTag(identifier)) {
    ...
    return w(prettyIdentifier, defaultValue);
  }
  return wt(defaultValue) ? w(ELEMENT_CAPTION, defaultValue) : w(defaultValue);
}
...
function isPlainObject(object) {
  ...
  if (Object.prototype.toString.call(object) !== "[object Object]") {
    ...
    return tag == null || !Object.getOwnPropertyDescriptor(object, Symbol.toStringTag)?.writable ? false : object.toString() === `[object ${tag}]`;
  }
  let proto = object;
...
var stringResolver = (rawDefaultProp) => w(JSON.stringify(rawDefaultProp));
...
var functionResolver = (rawDefaultProp, propDef) => {
  ...
  if (funcName != null) {
    if (isElement) return w(getPrettyElementIdentifier(funcName));
    inspectionResult != null && (inspectionResult = inspectValue(rawDefaultProp.toString()));
    let { hasParams } = inspectionResult.inferredType;
    return w(getPrettyFuncIdentifier(funcName, hasParams));
  }
  return w(isElement ? ELEMENT_CAPTION : FUNCTION_CAPTION);
};
var defaultResolver = (rawDefaultProp) => w(rawDefaultProp.toString());
  ...
  try {
    switch (type.name) {
      ...
      case "arrayOf": {
        ...
        return compact != null && !Ke(compact) ? w(compact) : full ? w(short, full) : w(short);
      }
      case "func": {
        let { short, full } = generateType(type, extractedProp), summary = short, detail;
        return full && full.length < MAX_FUNC_LENGTH ? summary = full : full && (detail = toMultilineSignature(full)), w(summary, detail);
      }
      default:
        return null;
    }
...
function getPropDefs(component, section) {
  ...
  !at(component) && !component.propTypes && isMemo(component) && (processedComponent = component.type);
  let extractedProps = aa(processedComponent, section);
...
var parameters = { docs: { extractArgTypes, extractComponentDescription: ia } };
var argTypesEnhancers = [ya];

// node_modules/@storybook/react/dist/chunk-OZUYIFQK.mjs
function __definePreview(input) {
  let preview = tc({ ...input, addons: [entry_preview_exports, entry_preview_argtypes_exports, entry_preview_docs_exports, ...input.addons ?? []] }), defineMeta = preview.meta.bind(preview);
...
var INTERNAL_DEFAULT_PROJECT_ANNOTATIONS = (0, import_preview_api.composeConfigs)([entry_preview_exports, entry_preview_argtypes_exports, { renderToCanvas: async (renderContext, canvasElement) => {
